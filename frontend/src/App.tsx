...
export default function App() {
  ...
  return (
    <div className="app">
      <Cabecalho totalDeItens={totalDeItens} aoAbrirCarrinho={() => setDrawerAberto(true)} />
        ...
        {carrinho && (
          <PainelCarrinho
            ...
            aoAplicarCupom={(codigo) =>
              executar(() => api.aplicarCupom(carrinho.id, codigo), `Cupom ${codigo.toUpperCase()} aplicado.`)
            }
            aoRemoverCupom={() => executar(() => api.removerCupom(carrinho.id))}
            aoFinalizar={() =>
              executar(() => api.finalizarCarrinho(carrinho.id), 'Compra finalizada com sucesso!')
            }
